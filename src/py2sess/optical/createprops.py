...
def load_createprops_provider(path: str | Path, *, kind: str) -> dict[str, np.ndarray]:
    ...
    The provider directory contains physical or RT-adjacent inputs generated by
    CreateProps, not final RT intermediates such as ``tau``, ``omega``,
    ``asymm``, ``scaling``, or ``fo_exact_scatter``.
    """
...
def _aerosol_interp_from_wavelengths(wavelengths: np.ndarray) -> np.ndarray:
...
def _parse_uv_dump(dump_path: Path) -> dict[str, np.ndarray]:
    ...
    arrays = {
        ...
        "aerosol_interp_fraction": _aerosol_interp_from_wavelengths(wavelengths),
    }
    arrays.update(
